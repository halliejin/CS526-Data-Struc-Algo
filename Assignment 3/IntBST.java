...
public class IntBST extends NodeBinaryTree<Integer> {
	...
	public static IntBST makeBinaryTree(int[] a){
		 IntBST tree = binaryTreeBuilder(a, 0, a.length - 1);
		 return tree;
	}
	...
	public static IntBST binaryTreeBuilder(int[]a, int startIndex, int endIndex) {
		  ...
		  //base case
		  if (endIndex < startIndex) return tree;
		  //find the middle element of the array
		  int mid = startIndex + (endIndex - startIndex) / 2;
		  //add root note using the middle element to tree
		  Node<Integer> rt = tree.addRoot(a[mid]);
		  //build the left side of the tree using recursion
		  IntBST left = binaryTreeBuilder(a, startIndex, mid - 1);
		  //build the right side of the tree using recursion
		  IntBST right = binaryTreeBuilder(a, mid + 1, endIndex);
		  //attach the left and right tree ti the middle node tree
		  tree.attach(rt, left, right);
		  return tree;
	}
